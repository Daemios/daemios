...
function onGlobalDragStart(payload) {
  try {
    ...
    if (props.type === 'ability') {
      ...
      if (es.includes('ability') || (sid && (es === sid || es.includes(sid) || sid.includes(es)))) {
...
function onDrop(e) {
  try {
    ...
    emit('dropped', { payload, slot: { slotId: props.slotId, type: props.type } });
  } catch (err) {
    // ignore
  }
}
...
.slot-cell.is-small { min-height: 48px; }
...
.slot-cell .empty-icon { opacity:0.12; width:60%; height:60%; }
.slot-cell .drag-highlight {
  position: absolute; inset: 0; border-radius: 6px; pointer-events: none;
  box-shadow: inset 0 0 0 4px rgba(33,150,243,0.85), inset 0 0 18px rgba(33,150,243,0.35);
  transition: opacity 120ms ease-in-out;
}
.slot-type-pack { grid-column: span 2; grid-row: span 2; min-height: 104px; }
@media (max-width: 640px) { .slot-type-pack { grid-column: span 1; grid-row: span 1; min-height: 96px; } }
</style>

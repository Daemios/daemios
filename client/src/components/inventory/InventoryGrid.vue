...
const slots = computed(() => {
  ...
  (props.container?.items || []).forEach((it) => {
    if (typeof it.containerIndex === 'number') itemsByIndex[it.containerIndex] = it;
  });
...
.slot-item img { max-width: 100%; max-height: 36px; display:block; margin: 0 auto; }
.item-name { font-size: 0.75rem; text-align:center; }
.item-qty { font-size: 0.7rem; text-align:center; }

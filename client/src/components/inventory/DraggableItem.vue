    ...
    try {
      ...
      if (props.item && props.item.img) {
        ...
        img.onload = () => {
          ...
          try { dt.setDragImage(canvas, size / 2, size / 2); } catch (_) { /* ignore */ }
        };
        img.onerror = () => {
          ...
          ctx.fillText(props.label || props.item.name || '?', size / 2, size / 2);
          try { dt.setDragImage(canvas, size / 2, size / 2); } catch (_) { /* ignore */ }
        };
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(props.label || (props.item && props.item.name) || '?', size / 2, size / 2);
        try { dt.setDragImage(canvas, size / 2, size / 2); } catch (_) { /* ignore */ }
      }
    } catch (imgErr) {

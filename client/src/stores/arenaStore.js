...
export const useArenaStore = defineStore("arena", {
  ...
  actions: {
    setActive(args) {
      if (
        this.entities &&
        this.entities[this.activeRegister.x] &&
        this.entities[this.activeRegister.x][this.activeRegister.y]
      ) {
        this.entities[this.activeRegister.x][
          this.activeRegister.y
        ].active = false;
      }
      this.activeRegister = args;
      if (
        this.entities &&
        this.entities[args.x] &&
        this.entities[args.x][args.y]
      ) {
    ...
    registerEntity(args) {
      this.entityRegistry.push({ x: args.x, y: args.y });
    },
    clearEntityRegistry() {
      this.entityRegistry = [];
    },
    ...
    appendPlannedPath(cellPosition) {
      this.plannedPath.push(cellPosition);
    },
    clearPlannedPath() {
      this.plannedPath = [];
    },
    ...
    entityMouseOver(args) {
      if (this.entities?.[args.x]?.[args.y])
        this.entities[args.x][args.y].hover = true;
    },
    entityMouseOut(args) {
      if (this.entities?.[args.x]?.[args.y])
        this.entities[args.x][args.y].hover = false;
    },
    buildOverlays() {},
    setOverlay(args) {
      if (args.overlay) {
        ...
        console.log(
          "Tried to register overlay but overlay type was not provided"
        );
      }
    },
    ...
    clearOverlayAt(args) {
      if (this.overlayRegistry[args.overlay]) {
        if (this.overlays[args.x]?.[args.y])
          this.overlays[args.x][args.y][args.overlay] = false;
      }
      const index = this.overlayRegistry[args.overlay].findIndex(
        (entry) => entry.x === args.x && entry.y === args.y
      );
    ...
    generateDummyArena(size = 15) {
      ...
      if (!entities[this.activeRegister.x])
        entities[this.activeRegister.x] = [];
      ...
      this.registerEntity({
        x: this.activeRegister.x,
        y: this.activeRegister.y,
      });
    ...
    moveByCell() {
      if (this.confirmedPath.length > 0) {
        ...
        setTimeout(() => {
          const registryIndex = this.entityRegistry.findIndex(
            (entry) =>
              entry.x === this.activeRegister.x &&
              entry.y === this.activeRegister.y
          );
          if (registryIndex !== -1)
            this.entityRegistry[registryIndex] = { x, y };
          if (this.entities) {

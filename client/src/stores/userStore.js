...
export const useUserStore = defineStore("user", {
  ...
  actions: {
    ...
    setCharacter(character) {
      ...
      try {
        console.debug(
          "[userStore] setCharacter",
          character && { id: character.id, equipped: character.equipped }
        );
      } catch (e) {
    ...
    setCharacterAndInventory(character, inventory, options = {}) {
      ...
      const updatedContainerIds = Array.isArray(options.updatedContainerIds)
        ? options.updatedContainerIds
        : [];
      ...
      if (Array.isArray(inventory)) {
        if (capacityUpdated) {
          ...
          const updatedSet = new Set(
            updatedContainerIds.map((id) => String(id))
          );

          // Replace matching containers in-place, preserving order of existing inventory
          const existing = Array.isArray(this.inventory)
            ? this.inventory.slice()
            : [];
          ...
          try {
            console.debug("[userStore] replaced containers", {
              updatedContainerIds,
      ...
      try {
        console.debug("[userStore] setCharacterAndInventory", {
    ...
    async ensureInventory(force = false) {
      ...
      if (!force && Array.isArray(this.inventory) && this.inventory.length > 0)

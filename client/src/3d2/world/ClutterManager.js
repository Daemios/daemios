...
export default class ClutterManager {
  ...
  commitInstances({ layoutRadius = 1, contactScale = 1, hexMaxY = 1, modelScaleY = () => 1, filter = null, offsetRect = null, axialRect = null } = {}) {
  ...
  const rMax = axialRect?.rMax ?? (offsetRect?.rowMax ?? bounds.maxR);
    ...
    // use contactScale to slightly modulate spacing
    const hexWidthFactor = 1.5 * contactScale;
    const hexHeightFactor = Math.sqrt(3) * contactScale;
  ...
  for (const t of this.types) {
          ...
          if (next() < density) {
            const offx = (next() - 0.5) * layoutRadius * 1.0;
            const offz = (next() - 0.5) * layoutRadius * 1.0;
    ...
    for (const [id, list] of placements) {
      ...
      for (let i = 0; i < list.length; i++) {
        const it = list[i];
            const x = layoutRadius * hexWidthFactor * it.q + it.offx;
            const z = layoutRadius * hexHeightFactor * (it.r + it.q / 2) + it.offz;
        const y = hexMaxY * modelScaleY(it.q, it.r) + 0.005 * layoutRadius;
        pos.set(x, y, z);
        quat.setFromAxisAngle(axisY, it.yaw);
        sclV.setScalar(it.scl * layoutRadius);
        mat4.compose(pos, quat, sclV);

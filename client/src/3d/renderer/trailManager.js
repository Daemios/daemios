...
export function snapshotTrail(ctx, delayMs = 3000) {
  if (!ctx || !ctx.topIM || !ctx.sideIM || !ctx.trailTopIM || !ctx.trailSideIM) return;
  // Cancel any in-progress trail copy job
  if (ctx._trailCopy && ctx._trailCopy.cancel) {
    try { ctx._trailCopy.cancel(); } catch (e) { console.debug('trail copy cancel failed', e); }
    ctx._trailCopy = null;
  }
  ...
  try {
    ctx.trailTopIM.instanceMatrix.array.set(ctx.topIM.instanceMatrix.array.subarray(0, count * 16), 0);
    ctx.trailSideIM.instanceMatrix.array.set(ctx.sideIM.instanceMatrix.array.subarray(0, count * 16), 0);
    ...
    if (ctx.topIM.instanceColor && ctx.trailTopIM.instanceColor) {
      ctx.trailTopIM.instanceColor.array.set(ctx.topIM.instanceColor.array.subarray(0, count * 3), 0);
      ctx.trailTopIM.instanceColor.needsUpdate = true;
    }
    if (ctx.sideIM.instanceColor && ctx.trailSideIM.instanceColor) {
      ctx.trailSideIM.instanceColor.array.set(ctx.sideIM.instanceColor.array.subarray(0, count * 3), 0);
      ctx.trailSideIM.instanceColor.needsUpdate = true;
    }
  } catch (e) {
    console.debug('trail snapshot matrix copy failed', e);
  }
  if (ctx.trailTimer) { clearTimeout(ctx.trailTimer); ctx.trailTimer = null; }
  ctx.trailTimer = setTimeout(() => {
    ...
    if (ctx.chunkManager) {
      ...
      if (typeof ctx.chunkManager._prevNeighborhoodRect !== 'undefined') ctx.chunkManager._prevNeighborhoodRect = null;
      if (ctx.chunkManager.commitClutterForNeighborhood) ctx.chunkManager.commitClutterForNeighborhood();
    }
    if (ctx._trailCopy && ctx._trailCopy.cancel) { try { ctx._trailCopy.cancel(); } catch (e) { console.debug('trail copy cancel failed', e); } ctx._trailCopy = null; }

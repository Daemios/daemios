...
// Normalize default vs named exports across package builds
...
function sfc32(a, b, c, d) {
  // Use mutable locals to preserve PRNG state without reassigning parameters
  let A = a >>> 0;
  let B = b >>> 0;
  let C = c >>> 0;
  let D = d >>> 0;
...
function hash2i(x, y, seed) {
  ...
  let a = (x | 0) * 374761393 + (seed | 0) * 668265263;
  a |= 0;
  let b = (y | 0) * 1274126177 + 974711;
  b |= 0;
  a ^= a << 13;
  a ^= a >>> 17;
  a ^= a << 5;
...
export const Archetype = {
  EquatorialWet: "EquatorialWet",
  SubtropicalDryWest: "SubtropicalDryWest",
  SubtropicalMonsoonEast: "SubtropicalMonsoonEast",
  TemperateMaritime: "TemperateMaritime",
  TemperateContinental: "TemperateContinental",
  BorealPolar: "BorealPolar",
};
...
function chooseArchetype(cellId, lat01) {
  ...
  switch (roll) {
    case 0:
      return Archetype.SubtropicalDryWest;
    case 1:
      return Archetype.SubtropicalMonsoonEast;
    case 2:
      return Archetype.TemperateMaritime;
    case 3:
      return Archetype.TemperateContinental;
    case 4:
      return lat01 > 0.6
        ? Archetype.EquatorialWet
        : Archetype.TemperateMaritime;
    default:
      return lat01 < 0.35
        ? Archetype.BorealPolar
        : Archetype.TemperateContinental;
  }
}

// Biome chooser (major) matrix with elevation gating
function pickBiomeMajor({
  elevBand,
  tBand,
  mBand,
  slope,
  ...
  if (elevBand === ElevationBand.Coast) {
    ...
    if (
      tBand === TemperatureBand.Tropical &&
      (mBand === MoistureBand.Humid || mBand === MoistureBand.Saturated) &&
      slope < 0.35
    )
      return "MangroveCoast";
  ...
  const dryBias =
    archetype === Archetype.SubtropicalDryWest ||
    archetype === Archetype.TemperateContinental
      ? 1
      : 0;
  const wetBias =
    archetype === Archetype.EquatorialWet ||
    archetype === Archetype.TemperateMaritime ||
    archetype === Archetype.SubtropicalMonsoonEast
      ? 1
      : 0;
  ...
  if (tBand === TemperatureBand.Tropical) {
    if (
      mBand === MoistureBand.Saturated ||
      (mBand === MoistureBand.Humid && wetBias)
    )
      return "Rainforest";
  ...
  if (tBand === TemperatureBand.Cold) {
    if (mBand === MoistureBand.Saturated && slope < 0.35)
      return "BorealWetlandBog";
...
function pickBiomeSub({ major, slope, relief, tBand }) {
  switch (major) {
    case "Rainforest":
      return relief > 0.6 && tBand === TemperatureBand.Tropical
        ? "CloudForest"
        : "LowlandRainforest";
    case "TemperateForest":
      return relief > 0.55 ? "MixedForest" : "BroadleafForest";
    case "Savanna":
      return relief > 0.5 ? "DrySavanna" : "MoistSavanna";
    case "SteppeShrubland":
      return slope > 0.55 ? "RockySteppe" : "PrairieSteppe";
    case "DesertFringe":
      return slope < 0.35
        ? "ErgDunes"
        : relief > 0.55
        ? "RockyMesa"
        : "RegGravel";
    case "ColdDesertSteppe":
      return relief > 0.6 ? "BadlandsScree" : "ColdSteppe";
    case "Taiga":
      return relief > 0.55 ? "MountainTaiga" : "LowlandTaiga";
    case "BorealWetlandBog":
      return "PeatBog";
    case "AlpineMeadow":
      return relief > 0.65 ? "Subalpine" : "Montane";
    case "BareRockScree":
      return "Scree";
    case "SnowIce":
      return "Glacier";
    case "SandyCoast":
      return "BarrierLagoon";
    case "RockyCoast":
      return "CliffCoast";
    case "FjordCoast":
      return "DeepFjord";
    case "MangroveCoast":
      return "TidalMangrove";
    case "DeepOcean":
      return "Abyssal";
    case "CoralSea":
      return "BarrierReef";
...
export function createHexGenerator(seed) {
  ...
  const seedInt = hash2i(
    0x9e37,
    0x85eb,
    0xc2b2 ^ hash2i(13, 17, String(seed).length)
  );
  ...
  let warpFreq = 0.004,
    warpAmp = 80; // domain warp for bold coasts
  let baseFreq = 0.0065,
    baseShape = 1.5; // continental interiors contrast
  let ridgeFreq = 0.016; // mountain chains
  let detailFreq = 0.045,
    detailAmp = 0.18; // breakup
  ...
  function applyTuning() {
    ...
    if (
      tuning.oceanEncapsulation != null &&
      isFinite(tuning.oceanEncapsulation)
    ) {
  ...
  function simplex2(noise, x, y, f) {
    return noise.noise2D(x * f, y * f);
  }
  function n01(noise, x, y, f) {
    return 0.5 * (simplex2(noise, x, y, f) + 1);
  }
  ...
  function elevationAt(x, y) {
    ...
    let cont = Math.pow(clamp01(cont1 * 0.7 + cont2 * 0.3), baseShape * 0.95);
    ...
    const landPotential = clamp01(cont * 0.7 + plateInterior * 0.3);
    ...
    const seaCorridor = smoothstep(0.45, 0.9, edgeInv);
    // Oceans are primarily corridors through land with reduced base-ocean weight
    let oceanMask = clamp01(
      (1 - landMaskCore) * 0.38 + seaCorridor * landMaskCore * 1.2
    );
    ...
    small *= 0.6 + 0.4 * landMask;
    // Interior lift to puff up continental cores
    const interiorLift = landMask * edge * 0.1;
    ...
    const hOcean =
      cont * 0.55 + small * 0.65 - oceanMask * 0.05 - corridorDepth * 0.12; // deeper along corridors
    const hLand =
      cont * 0.86 + ridgeTerm * 0.52 + small * 1.08 + macroBias + interiorLift;
    ...
    if (oceanEncapsulation > 0) {
      ...
      const liftLand = oceanEncapsulation * 0.1;
    ...
    const riftEligible =
      (hash2i(elevCellIdSeed(xw, yw, seedInt), seedInt, 0x5bd1e995) >>> 0) %
        7 ===
      0;
    if (riftEligible) {
  ...
  function climateAt(x, y, elevMacro) {
    ...
    const latWarp =
      simplex2(noises.detail, x, y, baseFreq * 0.35) * (latPeriod * 0.06);
    const latCoord = latCoord0 + latWarp;
    const latBand =
      Math.abs(((((latCoord / latPeriod) % 1) + 1) % 1) - 0.5) * 2; // 0 at equator stripe, 1 at poles
    ...
    let windDir = {
      x: trades.x * (1 - w1) + west.x * w1,
      y: trades.y * (1 - w1) + west.y * w1,
    };
    windDir = {
      x: windDir.x * (1 - w2) + polar.x * w2,
      y: windDir.y * (1 - w2) + polar.y * w2,
    };
    ...
    if (grad > 0)
      moisture = clamp01(
        moisture + grad * 0.8
      ); // wetter windward (slightly stronger)
  ...
  function renderHints({ elevBand, h, moisture, temp, slope, shelfDepth01 }) {
    ...
    if (elevBand === ElevationBand.Shelf)
      bathymetryStep =
        shelfDepth01 < 0.25
          ? 1
          : shelfDepth01 < 0.5
          ...
          ? 3
          : 4;
    else if (elevBand === ElevationBand.DeepOcean)
      bathymetryStep =
        h < ElevationThresholds.deep * 0.4
          ? 6
          : h < ElevationThresholds.deep * 0.7
          ? 5
          : 4;
    ...
    const rockExposure = clamp01(
      slope * 0.75 + (h > ElevationThresholds.high ? 0.25 : 0)
    );
  ...
  function layer2(x, y, elev) {
    ...
    switch (archetype) {
      case Archetype.EquatorialWet:
        clim = applyDelta(clim, +0.07, +0.18, archetypeWeight);
        break;
      case Archetype.SubtropicalDryWest:
        clim = applyDelta(clim, 0.0, -0.18, archetypeWeight);
        break;
      case Archetype.SubtropicalMonsoonEast:
        clim = applyDelta(clim, 0.0, +0.12, archetypeWeight);
        break;
      case Archetype.TemperateMaritime:
        clim = applyDelta(clim, 0.0, +0.1, archetypeWeight);
        break;
      case Archetype.TemperateContinental:
        clim = applyDelta(clim, 0.0, -0.15, archetypeWeight);
        break;
      case Archetype.BorealPolar:
        clim = applyDelta(clim, -0.1, 0.0, archetypeWeight);
        break;
      default:
        break;
    }
    ...
    if (oddRoll === 0)
      ...
      clim = applyDelta(clim, -0.12, -0.1, archetypeWeight * 0.9); // cold pocket
  ...
  function layer3(elev, slope, clim, archetype) {
    ...
    const shelfDepth01 =
      elevBand === ElevationBand.Shelf
        ? clamp01(
            (elev.h - ElevationThresholds.deep) /
              (ElevationThresholds.shelf - ElevationThresholds.deep)
          )
        : 0;
    ...
    const shelfKind =
      elevBand === ElevationBand.Shelf
        ? tBand === TemperatureBand.Tropical
          ? "CoralSea"
          : tBand === TemperatureBand.Polar
          ? "IceMarginalSea"
          : "KelpShelf"
        : null;
    const major = pickBiomeMajor({
      elevBand,
      tBand,
      mBand,
      slope,
  ...
  function layer4(x, y, elev, slope, bio) {
    ...
    const volcanic =
      hashLocal % 1993 === 0 || (elev.ridge > 0.9 && hashLocal % 11 === 0);
    const basinness = slope < 0.22 && elev.h < Math.max(0.18, elev.cont - 0.02);
    const inlandSea =
      basinness && elev.h < 0.14 && (hashLocal >>> 6) % 73 === 0;
    const lake =
      !inlandSea &&
      basinness &&
      mBand !== MoistureBand.Arid &&
      (hashLocal >>> 8) % 53 === 0;
    const karst =
      (mBand === MoistureBand.Humid || mBand === MoistureBand.Saturated) &&
      slope > 0.25 &&
      slope < 0.6;
    const badlands =
      (mBand === MoistureBand.SemiArid || mBand === MoistureBand.Arid) &&
      slope > 0.55 &&
      major.indexOf("Desert") >= 0;
    const saltFlats =
      basinness &&
      (mBand === MoistureBand.Arid || mBand === MoistureBand.SemiArid) &&
      slope < 0.2;
    const fjord =
      elevBand === ElevationBand.Coast && coldness > 0.55 && slope > 0.55;
    const mangrove =
      elevBand === ElevationBand.Coast &&
      tBand === TemperatureBand.Tropical &&
      mBand !== MoistureBand.Arid &&
      slope < 0.4;
    const wetland =
      mBand === MoistureBand.Saturated &&
      slope < 0.3 &&
      (elevBand === ElevationBand.Lowland || lake);
    const rift = !!elev.riftEligible && elev.edge > 0.6;
    const plateau =
      (elevBand === ElevationBand.Highland ||
        elevBand === ElevationBand.Mountain) &&
      slope < 0.22 &&
      elev.h > ElevationThresholds.high;
    const marshRing =
      lake &&
      (mBand === MoistureBand.Humid || mBand === MoistureBand.Saturated);
    return {

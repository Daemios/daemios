...
export default class WorldGrid {
  constructor(opts = {}) {
    ...
    if (this._generatorTuning && this.hexGen.setTuning)
      this.hexGen.setTuning(this._generatorTuning);
  ...
  setGeneratorTuning(tuning) {
    this._generatorTuning = {
      ...(this._generatorTuning || {}),
      ...(tuning || {}),
    };
    if (this.hexGen && this.hexGen.setTuning)
      this.hexGen.setTuning(this._generatorTuning);
  ...
  getCell(q, r) {
    ...
    const s =
      this.generationScale &&
      isFinite(this.generationScale) &&
      this.generationScale > 0
        ? this.generationScale
        : 1.0;
    const qg = q / s;
    const rg = r / s;
    ...
    const waterMask =
      (this.waterMaskNoise.noise2D((q + 250) * 0.035, (r - 120) * 0.035) + 1) /
      2;
    ...
    if (elevBand === "Highland") {
      exaggeration =
        (exConf.highland ?? 1.2) + (exConf.ridgeBonus ?? 0.15) * (ridge * 0.4);
    } else if (elevBand === "Mountain") {
      exaggeration =
        (exConf.mountain ?? 1.6) + (exConf.ridgeBonus ?? 0.15) * (ridge * 0.8);

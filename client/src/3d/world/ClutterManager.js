...
export default class ClutterManager {
  constructor(options = {}) {
    ...
    this.densityMultiplier =
      options.densityMultiplier ?? CLUTTER_SETTINGS.densityMultiplier;
    this.avoidEdges = options.avoidEdges ?? CLUTTER_SETTINGS.avoidEdges;
    this.collisionFactorFlora =
      options.collisionFactorFlora ??
      CLUTTER_SETTINGS.collisionFactorFlora ??
      1.0;
        ...
        (gltf) => {
          ...
          src.traverse((child) => {
            if (child.isMesh && child.geometry) {
              ...
              const center = g.boundingSphere
                ? g.boundingSphere.center
                : new THREE.Vector3();
              let inward = 0,
                total = 0;
          ...
          if (!geom) {
            console.warn("[Clutter] No geometry in", t.id);
            resolve();
            return;
          }
          geom.computeBoundingSphere();
          const mat = new THREE.MeshLambertMaterial({
            color: t.baseColor || 0xffffff,
          ...
          mat.onBeforeCompile = (shader) => {
            ...
            shader.uniforms.uFadeEnabled = {
              value: self._fade.enabled ? 1 : 0,
            };
            shader.uniforms.uHexCornerRadius = { value: self._fade.corner };
            shader.uniforms.uCullWholeHex = {
              value: self._fade.cullWholeHex ? 1 : 0,
            };
            const vDecl =
  ...
  _scheduleCommitTick(token) {
    ...
    if (typeof requestIdleCallback === "function") {
      this._idleId = requestIdleCallback(run, {
        timeout: Math.max(4, this.streamBudgetMs),
      });
  ...
  _processCommitTick(token) {
    ...
    const hexMaxY =
      job.opts && typeof job.opts.hexMaxY === "number" ? job.opts.hexMaxY : 1.0;
    const modelScaleYFn =
      typeof job.opts.modelScaleY === "function"
        ? job.opts.modelScaleY
        : () => 1.0;
    const tileInner = layoutRadius * contactScale * this.avoidEdges;
    const doRadialCullWholeHex =
      this._fade.enabled && this._fade.cullWholeHex && this._fade.radius > 0;
    ...
    if (job.phase === 1) {
      ...
      for (;;) {
        ...
        if (doRadialCullWholeHex) {
          ...
          if (minCenterDist + fadeCorner >= this._fade.radius) {
            if (
              (performance.now?.() ?? Date.now()) - tStart >=
              this.streamBudgetMs
            )
              break;
            else continue;
        ...
        if (typeof filter === "function" && !filter(q, r)) {
          if (
            (performance.now?.() ?? Date.now()) - tStart >=
            this.streamBudgetMs
          )
        ...
        for (const typeDef of this.types) {
          ...
          if (!rule) continue;
          if (
            typeDef.id === "tree_pine" &&
            cell.biome !== "deepWater" &&
            cell.biome !== "shallowWater"
          ) {
            if (temp > 0.45) continue;
          }
          if (
            typeDef.id === "tree_round" &&
            cell.biome !== "deepWater" &&
            cell.biome !== "shallowWater"
          ) {
          ...
          if (typeDef.category === "flora") {
            ...
            capacity = Math.max(
              0,
              Math.round(
                THREE.MathUtils.lerp(0, maxPerTile, Math.pow(fol, 1.1))
              )
            );
          } else {
          ...
          for (let k = 0; k < maxAttempts && placed < capacity; k += 1) {
            ...
            const yaw =
              typeDef.rotation && typeDef.rotation.yawRandom
                ? rng.nextIn(0, Math.PI * 2)
                : 0;
            ...
            const minDist =
              baseR *
              1.15 *
              (typeDef.category === "flora" ? this.collisionFactorFlora : 1.0);
            let ok = true;
            if (typeDef.category === "flora") {
              for (let j = 0; j < floraPlaced.length; j += 1) {
                ...
                if (
                  dx * dx + dz * dz <
                  (minDist + p.minDist) * (minDist + p.minDist)
                ) {
                  ok = false;
                  break;
                }
            ...
            if (typeDef.category === "flora")
              floraPlaced.push({ x: offx, z: offz, minDist });
            placed += 1;
          }
        }
        // iterated one tile
    ...
    if (job.phase === 3) {
      ...
      const doRadialCull =
        this._fade.enabled && this._fade.cullWholeHex && this._fade.radius > 0;
      const cx = this._fade.center.x;
      const cz = this._fade.center.y;
      const cornerR = this._fade.corner || 0.0;
      // Write across types with time budget
      for (;;) {
        if (job.write.typeIndex >= this.types.length) {
          break;
        }
        const t = this.types[job.write.typeIndex];
        const im = this.pools.get(t.id);
        if (!im) {
          job.write.typeIndex += 1;
          job.write.instIndex = 0;
          job.write.dstIndex = 0;
          job.write.maxWritable = 0;
          job.write.maxScl = 0;
          continue;
        }
        const list = job.placements.get(t.id);
        if (job.write.maxWritable === 0)
          job.write.maxWritable = Math.min(
        ...
        while (job.write.instIndex < job.write.maxWritable) {
          ...
          if (doRadialCull) {
            const dx = x - cx;
            const dz = z - cz;
            const cDist = Math.hypot(dx, dz);
            if (cDist + cornerR >= this._fade.radius) {
          ...
          if (y === undefined) {
            y = hexMaxY * modelScaleYFn(it.q, it.r) + 0.005 * layoutRadius;
            job.yCache.set(key, y);
          }
          _pos.set(x, y, z);
        ...
        if (doRadialCull && im.count > 0 && asset && asset.geom) {
          ...
          if (!asset.geom._daemiosSharedCloned) {
            const cloned = asset.geom.clone();
            cloned.boundingSphere = bs;
            im.geometry = cloned;
            asset.geom._daemiosSharedCloned = true;
          } else {
        ...
        const now2 =
          typeof performance !== "undefined" && performance.now
            ? performance.now()
            : Date.now();
        if (now2 - tStart >= this.streamBudgetMs) {
          this._scheduleCommitTick(token);
          return;
        }
      }
  ...
  setRadialFadeState({ enabled, center, radius, corner, cullWholeHex = true }) {
    ...
    const updateUniforms = (u) => {
      ...
      if (u.uFadeCenter && u.uFadeCenter.value)
        u.uFadeCenter.value.copy(this._fade.center);
      if (u.uFadeRadius) u.uFadeRadius.value = this._fade.radius;
      if (u.uHexCornerRadius) u.uHexCornerRadius.value = this._fade.corner;
      if (u.uCullWholeHex)
        u.uCullWholeHex.value = this._fade.cullWholeHex ? 1 : 0;
    };
    /* eslint-enable no-param-reassign */
    for (const [, asset] of this.assets) {
      const um =
        asset.material &&
        asset.material.userData &&
        asset.material.userData._fadeUniforms;
      updateUniforms(um);
      const ud =
        asset.depthMaterial &&
        asset.depthMaterial.userData &&
        asset.depthMaterial.userData._fadeUniforms;
      updateUniforms(ud);
    }
  }
}
